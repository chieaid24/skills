...
| **writing-style** | House style for published prose - anything a reader outside the work sees: READMEs, posts, release notes, announcements, docs, landing copy, PR bodies. Rules that fight a model's prose defaults (hype, throat-clearing, restating the diff, over-structuring, em dashes), plus an optional private voice overlay at `~/.agents/writing/voice/VOICE.md` that calibrates tone from your own writing samples. Stays out of the internal record: commit messages and code comments follow the repo's own convention. |
...
- `ui-audit` drives a browser: it reuses the repo's Playwright or Cypress config when there is
