...
## Notes
- **Constrained local mutation:** the only git write is `git pull --ff-only origin main` on a clean worktree already on `main`; never switch branches or stash, and ask the user first when the state is unsuitable. Every other write is confined to `progress/progress.md`, its dev-server log, and a `.gitignore` line. Starting and health-checking the dev server are the only process side effects; never run feature tests.
