...
**Authoring the queue**
- Turn context into a PRD with `to-prd`, then split with `to-issues` (batch) or add one with `/issue` (incremental) — one issue = one tracer-bullet vertical slice.
...
**`/next-issue` — the worker loop** (point each agent at this; it self-loops until stopped)
